...
export const startLidSyncJob = () => {
  const lidSyncJob = new CronJob(
    "0 */15 * * * *", // A cada 15 minutos (otimizado)
    async () => {
      try {
        const result = await syncContactLids();
        if (result.processed > 0) {
          logger.info(`[RDS-LID-SYNC] Job concluído: ${result.updated}/${result.processed} contatos sincronizados`);
        }
        // Se processed === 0, não loga nada (reduz I/O)
      } catch (error) {
        logger.error("[RDS-LID-SYNC] Erro no job:", error);
      }
    },
    null, 
    true,
  ...
  logger.info("[RDS-LID-SYNC] Job de sincronização de LIDs iniciado - rodará a cada 15 minutos");
