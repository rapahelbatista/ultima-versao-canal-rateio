...
export const createSubscription = async (
  ...
  if (key_GERENCIANET_PIX_KEY) {
    ...
    try {
      ...
      logger.error(`[SUBSCRIPTION] Erro Gerencianet PIX: ${error}`);
...
export const createWebhook = async (
  ...
  logger.info(`[WEBHOOK] createWebhook chamado`);
  ...
  try {
    ...
    logger.error(`[WEBHOOK] Erro ao criar webhook: ${error}`);
  }
};
...
export const mercadopagowebhook = async (
  ...
  logger.info(`[WEBHOOK] MercadoPago webhook recebido - action: ${req.body?.action}`);
  ...
  if (req.body.action === "payment.updated") {
    ...
    try {
      ...
      logger.info(`[WEBHOOK] Pagamento MP processado - fatura: ${payment.body.external_reference}`);

      if (!payment.body.transaction_details.transaction_id) {
        logger.info(`[WEBHOOK] Sem transação para fatura: ${payment.body.external_reference}`);
        return;
      }

      const invoices = await Invoices.findOne({ where: { id: payment.body.external_reference } });
      if (!invoices) {
        logger.warn(`[WEBHOOK] Fatura MP não encontrada: ${payment.body.external_reference}`);
        return;
      }
      const invoiceID = invoices.id;

      if (invoices.status === "paid") {
        logger.info(`[WEBHOOK] Fatura já paga: ${payment.body.external_reference}`);
        return;
      }
