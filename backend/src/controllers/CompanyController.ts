...
import ToggleCompanyStatusService from "../services/CompanyService/ToggleCompanyStatusService";
...
export const toggleStatus = async (
  ...
  if (!requestUser || !requestUser.super) {
    return res.status(401).json({ error: "Acesso não permitido. Apenas super admins." });
  }

  if (typeof status !== "boolean") {
    return res.status(400).json({ error: "O campo 'status' (true/false) é obrigatório." });
  }
