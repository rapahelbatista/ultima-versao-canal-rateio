...
export const createFromContacts = async (req: Request, res: Response): Promise<Response> => {
  const { companyId } = req.user;
  const { name, contactIds } = req.body as { name: string; contactIds: number[] };

  if (!name || !contactIds || !contactIds.length) {
    throw new AppError("Nome e contatos são obrigatórios", 400);
  }

  // Criar a lista de contatos
  const contactList = await CreateService({ name, companyId });

  // Buscar os contatos selecionados
  ...
  await ContactListItem.bulkCreate(items);

  // Recarregar com itens
  const record = await ContactList.findByPk(contactList.id, {
