...
export class FlowCampaignModel extends Model<FlowCampaignModel> {
  ...
  @Column({
    type: DataType.TEXT,
    get(this: FlowCampaignModel) {
      const rawValue = this.getDataValue('tagIds' as any);
      if (!rawValue) return [];
      try {
        const parsed = JSON.parse(rawValue as string);
    ...
    set(this: FlowCampaignModel, value: number[]) {
      if (Array.isArray(value) && value.length > 0) {
        this.setDataValue('tagIds' as any, JSON.stringify(value));
      } else {
        this.setDataValue('tagIds' as any, null);
      }
    }
  })
  tagIds: number[];
  ...
  /**
   * Verifica se o contato tem pelo menos uma das tags da campanha
   * Retorna true se a campanha não tiver tags configuradas (sem filtro de tag)
   */
  matchesContactTags(contactTagIds: number[]): boolean {
    const campaignTagIds = this.tagIds || [];
    if (campaignTagIds.length === 0) {
      return true; // Sem filtro de tag = aceita todos
    }
    return campaignTagIds.some(tagId => contactTagIds.includes(tagId));
  }
