...
const verifyQueueOficial = async (
    ...
    const botText = async () => {
        if (choosenQueue || (queues.length === 1 && chatbot)) {
            ...
            // ✅ INICIA INTEGRAÇÃO TYPEBOT/DIALOGFLOW/N8N APÓS ESCOLHER FILA
    ...
    const botButton = async () => {
        // Para API Oficial com botões, usa botText pois botões interativos são tratados dentro de botText
        return botText();
    }
