...
const ListTicketsService = async ({
  ...
  if (status === "open") {
    ...
    if (status === "group" && user.allowGroup) {
      ...
      // Sem conexão vinculada = não vê nenhum grupo
      if (userWhatsappIds.length === 0) {
        whereCondition = {
          companyId,
          id: -1 as any // Forçar resultado vazio
        };
      } else {
        whereCondition = {
          companyId,
  ...
  if (status === "closed") {
    ...
    if (status === "search") {
      ...
      if (Array.isArray(tags) && tags.length > 0) {
        ...
        // Restringir visibilidade: usuários não-admin só veem seus próprios tickets + pendentes ao filtrar por tags
        if (user.profile === "user" && showAll !== "true") {
          whereCondition = {
            ...whereCondition,
            [Op.or]: [
              { userId },
      ...
      if (withUnreadMessages === "true") {
        ...
        if (status === "group" && (user.allowGroup || showAll === "true")) {
          const grpWhatsappIds: number[] = [];
          if (user.whatsapps && user.whatsapps.length > 0) {
            user.whatsapps.forEach((w: any) => grpWhatsappIds.push(w.id));
          }
          if (user.whatsappId && !grpWhatsappIds.includes(user.whatsappId)) {
            grpWhatsappIds.push(user.whatsappId);
          }
          if (grpWhatsappIds.length === 0) {
            // Sem conexão vinculada = não vê nenhum grupo
            whereCondition = {
              ...whereCondition,
              id: -1 as any
            ...
            whereCondition = {
              ...
              whatsappId: { [Op.in]: grpWhatsappIds }
            };
          }
        }
      }
