...
const ListUserQueueServices = async (queueId: string | number): Promise<UserQueue> => {
    ...
    // RANDOM: seleciona em memória apenas entre usuários ONLINE
    ...
    const onlineUserQueues = userQueues.filter(uq => (uq as any).user?.online === true);

    // Se nenhum usuário online, fallback para todos da fila
    const pool = onlineUserQueues.length > 0 ? onlineUserQueues : userQueues;
