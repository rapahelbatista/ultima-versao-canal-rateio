...
// handleOpenPendingTickets removido — lógica duplicada de handleOpenTickets
// handleOpenTickets já trata expiresTicket, timeInactiveMessage e flowInactiveTime
  ...
  try {
    ...
    if (whatsapps.length > 0) {
      for (const whatsapp of whatsapps) {
        // handleOpenTickets já trata expiresTicket E flowInactiveTime internamente
        if (whatsapp.expiresTicket || whatsapp.flowInactiveTime) {
          await handleOpenTickets(companyId, whatsapp);
        }
        if (whatsapp.expiresTicketNPS) {
          await handleNPSTickets(companyId, whatsapp);
        }
        // handleOpenPendingTickets removido — era duplicata de handleOpenTickets
