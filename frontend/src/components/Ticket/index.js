import React, { useState, useEffect, useContext, useRef, useCallback, Suspense, useMemo } from "react";
...
// ✅ Todos os React.lazy DEPOIS dos imports estáticos
...
const Ticket = () => {
  ...
  const [presenceState, setPresenceState] = useState(null); // "composing" | "recording" | null
      ...
      const fetchTicket = async () => {
        try {
          ...
          if (!isNil(ticketId) && ticketId !== "undefined") {
            ...
            // Verificar se há parâmetro 'tab' na URL - TEM PRIORIDADE ABSOLUTA
            const urlParams = new URLSearchParams(window.location.search);
            const tabParam = urlParams.get('tab');
            
            // Se houver parâmetro tab na URL, usar ele SEMPRE (tem prioridade absoluta)
            // NÃO usar o status do ticket se houver parâmetro na URL
            if (tabParam && ["pending", "open", "group", "closed"].includes(tabParam)) {
              // Usar setTimeout para garantir que seja aplicado depois que o TicketsManagerTabs carregar
              // E usar um delay maior para garantir que o TicketsManagerTabs já processou o parâmetro
              setTimeout(() => {
                setTabOpen(tabParam);
              }, 200);
            } else {
              // Só usar o status do ticket se NÃO houver parâmetro na URL
              // E apenas se o status for válido
  ...
  useEffect(() => {
    ...
    if (user.companyId) {
      ...
      // ✅ Listener de presença (digitando...)
      const onPresenceUpdate = (data) => {
        if (data.ticketId === ticket?.id) {
          setPresenceState(data.presence === "composing" || data.presence === "recording" ? data.presence : null);
      ...
      // ✅ presenceSubscribe automático ao abrir ticket
        ...
        <Suspense fallback={<CircularProgress size={24} />}>
          <TicketHeader loading={loading}>
            {ticket.contact !== undefined && (
              <div id="TicketHeader" style={{ overflow: "hidden", flexShrink: 1, minWidth: 0, maxWidth: "calc(100% - 120px)" }}>
