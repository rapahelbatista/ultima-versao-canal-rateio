...
const useStyles = makeStyles((theme) => ({
  ...
  toolbarIcon: {
    ...
    // Herda a mesma cor de fundo do drawer — sem fundo branco fixo
...
const LoggedInLayout = ({ children, themeToggle, hideMenu = false }) => {
  ...
  const handleDismissAnnouncement = (ann) => {
    ...
    // Se não sobrar nenhum, fecha o modal
  ...
  useEffect(() => {
    const fetchAnnouncements = async () => {
      try {
        ...
        // Filtra informativos não dispensados
        let storedDismissed = {};
        try {
          const stored = localStorage.getItem("announcementsDismissed");
          storedDismissed = stored ? JSON.parse(stored) : {};
        } catch {}
        
        const visibleAnnouncements = activeAnnouncements.filter(ann => {
          const dismissedAt = storedDismissed[ann.id];
          if (!dismissedAt) return true;
          return ann.updatedAt && ann.updatedAt !== dismissedAt;
        });

        // Mostra o modal apenas se houver informativos visíveis
          ...
          <div className={classes.toolbarIcon}>
            <img
              ...
              style={{
                ...
                backgroundColor: "transparent",
                objectFit: "contain",
                // screen: fundo branco da logo some em fundo escuro (branco×escuro = transparente visualmente)
                // normal: mantém cores reais no modo claro
                mixBlendMode: theme.mode === "dark" ? "screen" : "normal",
              }}
              alt="logo"
            />
          ...
          {selectedAnnouncement ? (
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 16 }}>
                <Button
                  onClick={() => handleDismissAnnouncement(selectedAnnouncement)}
                  color="default"
                  size="small"
