        ...
        >
          <div className={classes.header}>
            <div className={classes.leftHeader}>
              <Avatar src={ticket.contact.urlPicture} className={classes.avatar} style={{ backgroundColor: !ticket.contact.urlPicture ? '#3f51b5' : undefined, color: '#fff', fontWeight: 'bold' }}>
                {!ticket.contact.urlPicture && ticket.contact.name
                  ? ticket.contact.name.trim().split(/\s+/).slice(0, 2).map(w => w[0]?.toUpperCase()).join('')
                  : null}
              </Avatar>
